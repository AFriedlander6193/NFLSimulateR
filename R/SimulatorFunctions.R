...
# Function to load all .rda files once
...
PFF_data_grabber <- function(year, pos_file) {
  ...
  file_path <- file.path("inst",
                         "extdata",
                         paste0("PFF", pos_file, year, ".rda"))
  load(file_path, envir = data_env)

  # Access it dynamically
  data1 <- get(paste0("PFF_", pos_file, "_", year), envir = data_env)
...
loading_ext_data <- function() {
  files <- list.files(
    system.file("extdata", package = "NFLSimulator"),
    full.names = TRUE
  )
...
choose_personnel <- function(posstm, down, togo, YdsBef, posstmdiff, quarter_secs, quarter){
  ...
  probs <- predict(data_env$off_personnel_model, modeldat, type = "probs")
...
choose_def_personnel <- function(deftm, down, togo, YdsBef, posstmdiff, quarter_secs, quarter, personnel){
  ...
  probs <- predict(data_env$defpersonnelmodel, modeldat, type = "probs")
...
choose_formation <- function(posstm, down, togo, YdsBef, posstmdiff,
  ...
  probs <- predict(data_env$off_formation_model, modeldat, type = "probs")
  formations <- sort(unique(data_env$offdf$off_form))
...
choose_def_coverage <- function(deftm, down, togo, YdsBef, posstmdiff, quarter_secs,
  ...
  probs <- predict(data_env$defcoveragemodel, modeldat, type = "probs")
  coverages <- sort(unique(data_env$offdf$def_simple_coverage))
...
runorpass <- function(posstm, down, togo, YdsBef, posstmdiff, quarter_secs,
  ...
  passprob <- predict(data_env$runpass_model, modeldat, type = "response")
  runprob <- 1 - passprob
...
routeselection <- function(posstm, deftm, down, togo, YdsBef, posstmdiff, quarter_secs, quarter,
  ...
  if(runpassselection=="Pass"){
    ...
    coverx <- posterior_predict(data_env$coverplayersmodel, newdata = modeldat)
    coverplayers <- round(apply(coverx, 2, sample, size = 1),0)
    passrushers <- 11 - coverplayers

    ### FIGURES OUT THE NUMBER OF ROUTES RAN
    routesrunprobs <- predict(data_env$route_count_model, modeldat, type = "probs")
    ...
    off_depth_chart <- full_PFF(posstm, year)
    ...
    def_depth_chart <- full_PFF(deftm, year)
